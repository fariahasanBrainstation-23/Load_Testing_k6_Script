...
import { getCustomerActionCodes, getCustomerActivityLog } from './lib/activityLog.js';

const users = new SharedArray('users', function () {
  return JSON.parse(open('../data/users.json'));
});

const { logRequest, getLogs } = createRequestLogger('customer_activity_log_test.js');
...
export function handleSummary(data) {
  return {
    ...
    'reports/customer_activity_log-report.html': htmlReport(data),
    'reports/customer_activity_log-summary.txt': textSummary(data, { indent: ' ', enableColors: false }),
    'reports/summary.json': JSON.stringify(data, null, 2),
    'reports/customer_activity_log-log.json': JSON.stringify(getLogs(), null, 2),
  };
}
...
export default function () {
  ...
  const actionCodesRes = getCustomerActionCodes(authHeaders);
  logRequest({
    apiName: 'CUSTOMER_ACTIVITY_LOG_ACTION_CODES',
    requestMethod: 'GET',
  ...
  const unfilteredRes = getCustomerActivityLog(authHeaders, { pageNumber: 0, pageSize });
  logRequest({
    apiName: 'CUSTOMER_ACTIVITY_LOG',
    requestMethod: 'GET',
  ...
  if (username || actionCode || status) {
    const filteredRes = getCustomerActivityLog(authHeaders, { pageNumber: 0, pageSize, username, actionCode, status });
    logRequest({
      apiName: 'CUSTOMER_ACTIVITY_LOG_FILTERED',
      requestMethod: 'GET',
      url: filteredRes.url,
