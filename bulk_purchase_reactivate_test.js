...
const fileBinary = open('./Excel-files/purchase_Reactivate.xlsx', 'b');
const FILE_NAME = 'purchase_Reactivate.xlsx';

const { logRequest, getLogs } = createRequestLogger('bulk_purchase_reactivate_test.js');

// VUS + ITERATIONS_PER_VU: each VU runs ITERATIONS_PER_VU full reactivate flows independently.
...
export function handleSummary(data) {
  return {
    ...
    'reports/bulk_purchase_reactivate-report.html': htmlReport(data),
    'reports/bulk_purchase_reactivate-summary.txt': textSummary(data, { indent: ' ', enableColors: false }),
    'reports/summary.json': JSON.stringify(data, null, 2),
    'reports/bulk_purchase_reactivate-log.json': JSON.stringify(getLogs(), null, 2),
  };
}
...
export default function () {
  ...
  const bulkRes = triggerBulkUpload(authHeaders, filePath, 'PURCHASE_REACTIVE');
  logRequest({
