...
const fileBinary = open('./Excel-files/purchase_renewBulk.xlsx', 'b');
const FILE_NAME = 'purchase_renewBulk.xlsx';

const { logRequest, getLogs } = createRequestLogger('bulk_purchase_renew_test.js');

// VUS + ITERATIONS_PER_VU: each VU runs ITERATIONS_PER_VU full renew flows independently.
...
export function handleSummary(data) {
  return {
    ...
    'reports/bulk_purchase_renew-report.html': htmlReport(data),
    'reports/bulk_purchase_renew-summary.txt': textSummary(data, { indent: ' ', enableColors: false }),
    'reports/summary.json': JSON.stringify(data, null, 2),
    'reports/bulk_purchase_renew-log.json': JSON.stringify(getLogs(), null, 2),
  };
}
...
export default function () {
  ...
  const bulkRes = triggerBulkUpload(authHeaders, filePath, 'PURCHASE_RENEW');
  logRequest({
