...
const fileBinary = open('./Excel-files/purchase_change_planBulk.xlsx', 'b');
const FILE_NAME = 'purchase_change_planBulk.xlsx';

const { logRequest, getLogs } = createRequestLogger('bulk_purchase_changeplan_test.js');

// VUS + ITERATIONS_PER_VU: each VU runs ITERATIONS_PER_VU full change-plan flows independently.
...
export function handleSummary(data) {
  return {
    ...
    'reports/bulk_purchase_changeplan-report.html': htmlReport(data),
    'reports/bulk_purchase_changeplan-summary.txt': textSummary(data, { indent: ' ', enableColors: false }),
    'reports/summary.json': JSON.stringify(data, null, 2),
    'reports/bulk_purchase_changeplan-log.json': JSON.stringify(getLogs(), null, 2),
  };
}
...
export default function () {
  ...
  const bulkRes = triggerBulkUpload(authHeaders, filePath, 'PURCHASE_CHANGE_PLAN');
  logRequest({
