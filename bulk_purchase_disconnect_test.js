...
const fileBinary = open('./Excel-files/purchase_Disconnect.xlsx', 'b');
const FILE_NAME = 'purchase_Disconnect.xlsx';

const { logRequest, getLogs } = createRequestLogger('bulk_purchase_disconnect_test.js');

// VUS + ITERATIONS_PER_VU: each VU runs ITERATIONS_PER_VU full disconnect flows independently.
...
export function handleSummary(data) {
  return {
    ...
    'reports/bulk_purchase_disconnect-report.html': htmlReport(data),
    'reports/bulk_purchase_disconnect-summary.txt': textSummary(data, { indent: ' ', enableColors: false }),
    'reports/summary.json': JSON.stringify(data, null, 2),
    'reports/bulk_purchase_disconnect-log.json': JSON.stringify(getLogs(), null, 2),
  };
}
...
export default function () {
  ...
  const bulkRes = triggerBulkUpload(authHeaders, filePath, 'PURCHASE_DISCONNECT');
  logRequest({
